...
class dfedcata(Client):
    def __init__(self, device, model_func, received_vecs, dataset, lr, args):   
        super(dfedcata, self).__init__(device, model_func, received_vecs, dataset, lr, args)
        
        self.optimizer = torch.optim.SGD(self.model.parameters(), lr=lr, weight_decay= self.args.lamb + self.args.weight_decay)
    def train(self):
        # local training
        self.model.train()
